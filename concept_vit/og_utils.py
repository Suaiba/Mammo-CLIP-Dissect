...
def save_clip_target_activations(target_model, dataset, save_name, target_layers=["layer4"], batch_size=1000,
                            device="cuda", pool_mode='avg'):
    """
    save_name: save_file path, should include {} which will be formatted by layer names
    """
...
def save_activations(clip_name, target_name, target_layers, d_probe,
                     concept_set, batch_size, device, pool_mode, save_dir, breast_clip_ckh=None, fine_tuned_ckh=None,
                     args=None):
    ...
    if breast_clip_ckh is not None:
        print("MammoCLIP checkpoint provided")

        if fine_tuned_ckh is not None:
            print("Finetuned checkpoint provided")
            save_text = "/Latest_vindr_calc_finetuned_"
        else:
            finetuned_ckpt = None
            save_text = "/clip_dissector_{}_target_{}_small_mammo_pretrained_".format(target_name,d_probe)
    else:
        finetuned_ckpt = None
        print("No pre-trained or finetuned checkpoint provided")
        save_text = "/clip_dissector_{}_target_{}_small_not_mammo_pretrained_".format(target_name,d_probe)
    ...
    if target_name == "breastclip_classifier":
        ...
        target_model, target_preprocess = data_utils.get_target_model(target_name, device, args=args, ckpt=clip_ckpt,
    ...
    if target_name == "clip":
        target_save_name="saved_activations/clip_dissector_clip_imagenet_subsets_small_not_mammo_pretrained_saved_activations/imagenet_subsets_clip_{}.pt"
    ...
    if target_name != "clip":
        ...
        save_clip_target_activations(target_model, data_t, target_save_name, target_layers,
                                    batch_size, device, pool_mode)
...
def get_similarity_from_activations(target_save_name, clip_save_name, text_save_name, similarity_fn,
                                    return_target_feats=True, device="cuda", d_probe="vindr"):
    print("clip_save_name:", clip_save_name)
    print("text_save_name:", text_save_name)
