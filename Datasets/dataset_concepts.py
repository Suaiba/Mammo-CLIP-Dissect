...
class single_view_cancer_dataloader(nn.Module):
    def __init__(self,
                 root_folder: str,
                 annotation_csv: str,
                 imagefolder_path: str,args,
                 split: str = 'train',
                 transform=None):
        '''
        root_folder
        ...
        split: Either "training" or "test" split of dataset.
        transform:
        '''

        super().__init__()

        assert split in ['train', 'test', 'val'], 'split must be either "train" or "test" or "val"'

        annotation_csv = pd.read_csv(os.path.join(root_folder, annotation_csv))
        #self.annotation_csv = annotation_csv.loc[(annotation_csv['split'] == split) & (annotation_csv['Selected'] ==1)]
    ...
    def convert_series_to_numpy(self, df):
        return pd.Series.to_numpy(df)

    def __getitem__(self, idx):
        # study_id = self.unique_studyids[idx]

        # study_id_data = self.annotation_csv.loc[study_id, :]
        data_line = self.annotation_csv.iloc[idx]
        image_id = data_line["anon_filename"]
        #drop the .dcm extension
        image_id = image_id[:-4]
        if self.args.label.lower() == "cancer":
            label= data_line["Cancer_visible"]
        elif self.args.label.lower() == "density":
            label = data_line["BIRADS_density"]
        else:
            raise ValueError("Invalid label type")


        img_path = os.path.join(self.imagefolder_path, image_id + '.png')
        image = Image.open(img_path)
        #convert image to numpy array
        image = np.array(image, dtype=np.float32)
        # add axis at the end and repeat along it three times
...
class cancer_task_single_view_balanced_dataloader(nn.Module):
    def __init__(self,
                 root_folder: str,
                 annotation_csv: str,
                 imagefolder_path: str,args,
    ...
    def __len__(self):
        return len(self.balanced_subset)
    def convert_series_to_numpy(self, df):
        return pd.Series.to_numpy(df)
    def __getitem__(self, idx):
        ids = self.balanced_subset.iloc[idx, :]
        images = []
        labels = []
        for id in ids:
            if self.args.label.lower() == "cancer":
                label = self.annotation_csv.loc[id, 'Cancer_visible']
            elif self.args.label.lower() == "density":
                label = self.annotation_csv.loc[id, 'BIRADS_density']
            else:
                raise ValueError("Invalid label type")
            image_id = self.annotation_csv.loc[id, 'anon_filename']
            # drop the .dcm extension
            image_id = image_id[:-4]
            # Read the image
            img_path = os.path.join(self.imagefolder_path, image_id + '.png')
    ...
    def balanced_sample(self):
        new_df = self.annotation_csv.copy()
        unique_labels = np.unique(new_df['Cancer_visible'])
        # Sample each label with replacement to get a subset of the dataset
        subset = {}
        for i in unique_labels:
            image_ids = new_df[new_df['Cancer_visible'] == i].index.values
            sampled_image_ids = np.random.choice(image_ids, replace=True,
                                                 size=int(len(new_df) / len(unique_labels)))
            subset[i] = sampled_image_ids
        # Create a DataFrame with the sampled image_ids
        self.balanced_subset = pd.DataFrame(subset)
