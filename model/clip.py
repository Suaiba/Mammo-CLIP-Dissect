...
class BreastClip(nn.Module):
    def __init__(self, model_config: Dict, all_loss_config: Dict, tokenizer: PreTrainedTokenizer = None):
        ...
        if self.temperature:
            ...
            log.warning("[Mammo-CLIP] missing temperature scaling factor")
    ...
    def encode_text(self, text_tokens):
        # Ensure text_tokens is a dictionary
        if not isinstance(text_tokens, dict):
            raise ValueError("Text tokens must be a dictionary")
    ...
    def tokenize(self, texts: Union[str, List[str]], max_length: int = 256, padding: bool = True,
        ...
        padding : bool
            Whether to pad the sequences to the max_length
        truncation : bool
        ...
        if isinstance(texts, str):
            texts = [texts]
        return self.tokenizer(texts, max_length=max_length, padding=padding, truncation=truncation, return_tensors="pt")
